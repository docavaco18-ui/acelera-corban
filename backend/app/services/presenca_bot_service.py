"""Presença Bank bot service — mesma arquitetura VCTex."""
import asyncio
import json
import logging
...
async def _fetch_pending(db: Any, user_id: str, limit: int, batch_id: str | None = None) -> list[dict]:
    def _q():
        q = scoped(db, "presenca_leads", user_id).select("*").in_(
    ...
    if not rows and batch_id is not None:
        logger.warning("presenca batch %s vazia, buscando leads sem filtro de batch", batch_id)
        def _q_all():
            return scoped(db, "presenca_leads", user_id).select("*").in_(
                "status", ["pendente", "erro"]
...
async def start_bot(
    ...
    if batch_id is not None:
        try:
            ...
            logger.warning(f"presenca mark batch processando[{batch_id}] failed: {e}")
    ...
    async def on_event_async(event):
        ...
        await _broadcast(redis, {**event, "bank": "presenca"})
        on_event(event)
    ...
    async def _refill_queue():
        ...
        while rt.running:
            try:
                ...
                logger.info(
                    "presenca refill[%s] fetched=%d added=%d queue=%d inflight=%d batch=%s",
                ...
                logger.exception(f"presenca refill[{user_id}] error: {e}")
                await asyncio.sleep(REFILL_INTERVAL)
    ...
    async def _run():
        try:
            ...
            for i in range(handle.num_workers):
                ...
                async def _wrap(worker=w):
                    try:
                        ...
                        logger.info("presenca worker_%d encerrou", worker.worker_id)
                        active = [t for t in rt.worker_tasks if not t.done()]
                        if len(active) <= 1:
                            rt.inflight.clear()
                            logger.info("presenca todos workers encerraram; inflight limpo")
            ...
            def _finalize():
                ...
                scoped(db, "presenca_bot_runs", user_id).update({
                    "status": "completed",
                    "finished_at": now_iso,
                ...
                if batch_id is not None:
                    rows = (
                        scoped(db, "presenca_leads", user_id)
            ...
            try:
                ...
                logger.warning(f"presenca finalize[{user_id}] failed: {e}")
            await _broadcast(redis, {"type": "bot_status", "status": "idle",
                                     "user_id": user_id, "bank": "presenca"})
    ...
    return {
        "status": "started", "bank": "presenca",
        "run_id": handle.run_id, "full_workers": handle.num_workers,
    }


async def stop_bot(pool: PresencaBotPool, user_id: str):
    rt = _runtimes.get(user_id)
    if not rt:
        await pool.stop(user_id)
        return {"status": "not_running", "bank": "presenca"}
    rt.running = False
    if rt.run_task:
        rt.run_task.cancel()
        try:
    ...
    await _broadcast(redis, {"type": "bot_status", "status": "idle",
                             "user_id": user_id, "bank": "presenca"})
    return {"status": "stopped", "bank": "presenca"}


async def get_bot_status(pool: PresencaBotPool, user_id: str):
    handle = pool.status(user_id)
    if handle is None:
        return {"status": "idle", "bank": "presenca"}
    return {
        "status": "running",
        "bank": "presenca",
