...
from .v8_api_service import enrich_cpf, create_consent, authorize_consent, get_simulation_configs, pick_config, _max_installments, create_simulation, get_consult, V8APIError
...
class LeadWorker:
    def __init__(self, worker_id: int, redis, on_event: Callable, role: str = "full", role_index: int = 0):
    ...
    async def _update_lead(self, cpf: str, updates: dict):
        await asyncio.to_thread(
            lambda: db().table("v8_leads").update(updates).eq("cpf", cpf).execute()
        )
    ...
    async def _poll_consult(self, consult_id: str, interval: int = POLL_INTERVAL, timeout: int = POLL_TIMEOUT) -> dict:
        ...
        while loop.time() < deadline:
            try:
                data = await get_consult(consult_id, proxy=self.proxy)
    ...
    async def _finalize_terminal(self, cpf: str, consult_id: str, payload: dict):
        ...
        try:
            configs = await get_simulation_configs(proxy=self.proxy)
    ...
    async def process(self, lead: dict):
        ...
        try:
            ...
            client_data = await enrich_cpf(cpf, proxy=self.proxy)
            ...
            consult_id = await create_consent(cpf, client_data, telefone, proxy=self.proxy)
            await self._update_lead(cpf, {"status": "consentido", "consult_id": consult_id})
            self._emit("status_update", cpf=cpf, status="consentido")

            await authorize_consent(consult_id, proxy=self.proxy)
