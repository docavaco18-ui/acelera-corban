"""Upload de CSV pra Nossa Fintech — análogo ao vctex_upload_jobs."""
...
JOB_KEY_PREFIX = "nossafintech_upload:"
...
async def _create_batch(owner_id: str, file_name: str | None) -> str:
    ...
    name = file_name or f"Nossa Fintech Upload {uuid.uuid4().hex[:8]}"

    def _insert():
        return scoped(db(), "nossafintech_batches", owner_id).insert({
            "name": name,
            "file_name": file_name,
            "status": "pendente",
        }).execute()
...
async def _run(job_id: str, content: bytes, owner_id: str, batch_id: str) -> None:
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        try:
            def _upsert(b=batch):
                return (
                    scoped(db(), "nossafintech_leads", owner_id)
    ...
    try:
        def _update_batch():
            scoped(db(), "nossafintech_batches", owner_id).update({
