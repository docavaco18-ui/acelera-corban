"""Upload de CSV pra Presença Bank — análogo ao vctex_upload_jobs."""
...
JOB_KEY_PREFIX = "presenca_upload:"
...
async def _create_batch(owner_id: str, file_name: str | None) -> str:
    ...
    name = file_name or f"Presença Upload {uuid.uuid4().hex[:8]}"

    def _insert():
        return scoped(db(), "presenca_batches", owner_id).insert({
            "name": name,
            "file_name": file_name,
            "status": "pendente",
        }).execute()
...
async def _run(job_id: str, content: bytes, owner_id: str, batch_id: str) -> None:
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        try:
            def _upsert(b=batch):
                return (
                    scoped(db(), "presenca_leads", owner_id)
    ...
    try:
        def _update_batch():
            scoped(db(), "presenca_batches", owner_id).update({
