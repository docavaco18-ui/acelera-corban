"""Routers Presença Bank — leads, bot, stats, batches sob /api/presenca/*."""
import csv
import io
...
from ..banks.presenca.credentials_helper import get_presenca_runtime_creds
from ..services import presenca_upload_jobs, presenca_bot_service
...
async def upload_csv(
    ...
    return await presenca_upload_jobs.start_upload(content, user.user_id, file_name=file.filename)


@router.get("/leads/upload/{job_id}")
async def upload_status(job_id: str, _: AuthUser = Depends(require_user)):
    state = await presenca_upload_jobs.get_job(job_id)
    if state is None:
...
async def list_leads(
    ...
    db = get_db()
    q = scoped(db, "presenca_leads", user.user_id).select("*").order("created_at", desc=True)
    if status:
...
async def export_csv(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=presenca_{status}.csv"},
    )
...
async def bot_stop(request: Request, user: AuthUser = Depends(require_user)):
    pool = request.app.state.presenca_pool
    return await presenca_bot_service.stop_bot(pool, user.user_id)


@router.get("/bot/status")
async def bot_status(request: Request, user: AuthUser = Depends(require_user)):
    pool = request.app.state.presenca_pool
    return await presenca_bot_service.get_bot_status(pool, user.user_id)
...
def list_runs(limit: int = 20, user: AuthUser = Depends(require_user)):
    db = get_db()
    rows = (
        scoped(db, "presenca_bot_runs", user.user_id)
...
def batch_stats(batch_id: str, user: AuthUser = Depends(require_user)):
    ...
    return {
        ...
        "em_processamento": 0, "processados": eleg + ineleg + erros,
        "total_liberado": round(total_liberado, 2), "by_status": counts,
    }
...
def current_batch(user: AuthUser = Depends(require_user)):
    db = get_db()
    active = (
        scoped(db, "presenca_batches", user.user_id).select("*")
...
def delete_batch(batch_id: str, user: AuthUser = Depends(require_user)):
    ...
    if not own:
        raise HTTPException(404, "Batch não encontrada")
    scoped(db, "presenca_leads", user.user_id).delete().eq("batch_id", batch_id).execute()
    scoped(db, "presenca_batches", user.user_id).delete().eq("id", batch_id).execute()


@router.post("/leads/retry-errors")
def retry_errors(batch_id: str | None = None, user: AuthUser = Depends(require_user)):
    db = get_db()
    q = scoped(db, "presenca_leads", user.user_id).update({"status": "pendente", "erro": None}).eq("status", "erro")
    if batch_id:
        q = q.eq("batch_id", batch_id)
    result = q.execute()
