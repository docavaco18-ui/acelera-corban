...
function PageHeader({
  scoreNum, scoreStatus, scoreLabel, generatedAt, loading, onRefresh, onLiveAudit, liveTimedOut,
}: {
  scoreNum: number; scoreStatus: string; scoreLabel: string; generatedAt: string;
  loading: boolean; onRefresh: () => void; onLiveAudit: () => void; liveTimedOut: boolean;
}) {
    ...
    <div style={{ ...glassCard(grad, 28), marginBottom: 22 }}>
      <div style={{
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ ...sectionTitle(grad), marginBottom: 6 }}>Central de Controle</div>
...
export default function CentralControle() {
  ...
  const load = async (liveMeta = false) => {
    ...
    setError("");
    try {
      const overview = await commandCenterApi.overview({ live_meta: liveMeta });
      setData(overview);
    } catch (e: any) {
      ...
      <div style={{ minHeight: '100vh', background: C.bg, color: C.muted, padding: 32 }}>
        ...
        <div style={glassCard(G.primary, 32)}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <BrainBadge color={C.text} loading />
            <div>
    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, color: C.text, padding: 32 }}>
        ...
        <div style={glassCard(G.red, 28)}>
          ...
          <button onClick={() => load(false)} className="ds-btn" style={btnStyle(G.red)}>
            Tentar novamente
          </button>
        </div>
      </div>
...
function TemplateRow({ tpl }: { tpl: TemplateRow }) {
